<template>
  <div class>
    <el-form ref="form" :model="form" label-width="80px">
      ...
      <el-form-item label="是否轮播" class>
        <el-radio-group v-model="form.carousel" class>
          <el-radio :label="0">不轮播</el-radio>
          <el-radio :label="1">轮播</el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item label="活动类别" class>
        <el-radio-group v-model="form.activType" class>
          <el-radio :label="0">抽奖</el-radio>
          <el-radio :label="1">抢送</el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item label="活动性质" class>
        <el-radio-group v-model="form.conditionType" class @change="changeRadio">
          <el-radio :label="1">时间限制</el-radio>
          <el-radio :label="2">人数限制</el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item v-if="chaNum == 1" label>
        ...
        <el-date-picker
          ...
          @change="changeTime"
        ></el-date-picker>
        <el-input placeholder="请输入内容" v-model="form.conditionalDescription" class="sm-width-30-per"></el-input>
      </el-form-item>
      <el-form-item v-if="chaNum == 2" label>
        <el-input-number
          placeholder="参与人数"
          v-model="form.condition"
          @change="numberChange"
      ...
      <el-form-item label="参与条件">
        <el-input v-model="form.addCondition" class="sm-width-30-per" placeholder="参与条件"></el-input>
      </el-form-item>
      ...
      <el-form-item label="添加奖品">
        <div style="width:100%;height: 256px;" class="sm-layout-left-center">
          <div
            @click="addPrize"
            :class="borderStyle"
          ...
          >
            <img :src="item.iconUrl" style="width:100%;height:92px" />
            ...
            <el-button
              style="position:relative;left:76px;bottom: -12px;box-shadow: 0 5px 11px 0 rgba(0,0,0,.18), 0 4px 15px 0 rgba(0,0,0,.15);"
              type="danger"
              icon="el-icon-delete"
              circle
    ...
    <!-- 添加奖品模态框 -->
    <el-dialog title="添加奖品" :visible.sync="prizwFormVisible">
      <el-form :model="prizeForm">
        <el-form-item>
          <el-button type="primary" icon="el-icon-plus" circle @click="chosePrizw"></el-button>
        </el-form-item>
        <el-form-item>
          <!-- <div class="debug ms-layout-wrap-vertical"
          style="widht:256px;height:206px;">-->
          <img :src="prizeForm.prizeUrl" style="width:380px;height:230px;" />

          <!-- </div> -->
        </el-form-item>
        <el-form-item label="奖品名称" :label-width="formLabelWidth">
          <el-input v-model="prizeForm.prizeName" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="奖品数量" :label-width="formLabelWidth">
          <el-input v-model="prizeForm.prizeCount" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="获取条件" :label-width="formLabelWidth">
          <el-input v-model="prizeForm.ranking" autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="prizwFormVisible = false">取 消</el-button>
        <el-button type="primary" @click="cheosePrizeOk">确 定</el-button>
      </div>
    </el-dialog>

    <!-- 选择奖品模态框 -->
    <el-dialog title="选择奖品" :visible.sync="chosePrizwVisible">
      <div style="width:960px; height:560px;" class>
        <div class="sm-width-100-per sm-height-10-per sm-layout-center-horizontal">
          <el-autocomplete
            v-model="prizeDiscription"
        ...
        >
          ...
          >
            <img :src="item.iconUrl" style="width:190px;height:115px" />
            <div
              class="sm-width-100-per sm-layout-center-horizontal"
              style="height:26px;"
            >{{item.prizeDescription}}</div>
          </div>
          <el-alert
            title="已经到底部"
            type="info"
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        ...
        carousel: this.$route.query.carousel,
        activType: this.$route.query.activType
      },
  ...
  methods: {
    ...
    onSubmit() {
      ...
      let reForm = {};
      if (sponsorObj) {
        if (that.form.sponsorid != sponsorObj.id) {
          reForm.sponsorid = sponsorObj.id;
          reForm.sponsorName = sponsorObj.sponsorName;
          reForm.location = sponsorObj.location;
          reForm.address = sponsorObj.address;
      ...
      if (that.form.prizeList.length > that.prizListData.length) {
        ...
        for (var j = 0; j < that.prizListData.length; j++) {
          let flData = that.form.prizeList[j];
          let ad = false;
          for (var i = 0; i < that.form.prizeList.length; i++) {
            let pData = that.prizListData[i];
            if (flData.id == pData.id) {
              ad = true;
              break;
            }
      ...
      if (plist.length > 0) {
        reForm.prizeList = plist;
        reForm.prizeBool = true;
      } else {
        reForm.prizeBool = false;
      }
      // 广告富文本框
      if (that.form.adv != that.makdown.html) {
        reForm.adv = that.makdown.html;
        reForm.markdownAdv = that.makdown.content;
      }
      if (that.form.carousel != that.queryData.carousel) {
        reForm.carousel = that.form.carousel;
      }
      if (that.form.activType != that.queryData.activType) {
        reForm.activType = that.form.activType;
      }
      // 赞助商要求
      if (that.form.sponsorClaim != that.queryData.sponsorClaim) {
        reForm.sponsorClaim = that.form.sponsorClaim;
      }
      // 开讲条件描述开奖条件
      if (
        that.form.conditionalDescription !=
        that.queryData.conditionalDescription
      ) {
        reForm.conditionalDescription = that.form.conditionalDescription;
      }
      // 开奖条件
      if (that.form.condition != that.queryData.condition) {
        reForm.condition = that.form.condition;
      }
      // 开奖条件类型
      if (that.form.conditionType != that.queryData.conditionType) {
        reForm.conditionType = that.form.conditionType;
      }
      // 参与条件
      if (that.form.addCondition != that.queryData.addCondition) {
        reForm.addCondition = that.form.addCondition;
      }
      if (reForm) {
        reForm.id = that.form.id;
      } else {
        this.$message.error("修改数据后再提交！");
      }
      debugger;
      updateActivity(reForm)
        .then(res => {
          console.log(res);
          let data = res.data;
          if (data.code == -1) {
    ...
    chosePrizw() {
      ...
      let that = this;
      queryByPrizeDescription(that.PrizeObj.pageNum, that.PrizeObj.pageSize, "")
        .then(res => {
    ...
    cheosePrizeOk() {
      let that = this;
      if (
        !that.prizeForm.ranking ||
        !that.prizeForm.prizeCount ||
        !that.prizeForm.prizeUrl ||
        !that.prizeForm.prizeName
      ) {
        return;
      }
    ...
    querySearchAsync(queryString, cb) {
      ...
      queryByPrizeDescription(
        that.PrizeObj.pageNum,
        that.PrizeObj.pageSize,
        queryString
      )
        .then(res => {
          var resPonData = res.data;
