  ...
  <div>
    <div class="crumbs">
      ...
      <el-row>
        ...
        <el-button style="margin-left: 15px;"
                   type="primary"
                   icon="el-icon-search"
                   @click="conditionLimit(1)">时间限制</el-button>
        <el-button type="primary"
                   icon="el-icon-search"
                   @click="conditionLimit(2)">人数限制</el-button>

        <router-link style="margin-left:15px;"
                     :to="{path:'/addActiv',query: {name: '新增活动'}}">
          <el-button type="primary"
    ...
    <el-table :data="tableData"
      ...
      <el-table-column prop="address"
                       label="活动地点"></el-table-column>
      <el-table-column prop="conditionalDescription"
                       label="活动时间"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    getData () {
      let that = this;
      pageActivity(this.pageRequestObj).then(res => {
    ...
    handleStatus (row) {
      ...
      delActivity(row.id, state).then(res => {
        let resData = res.data;
        if (resData.code == 0) {
