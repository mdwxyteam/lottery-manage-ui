...
export default {
  ...
  data: function () {
    return {
      ...
      paddress:  this.$route.query.address,
      poiResult:null
    };
  },
  components: {
  ...
  methods: {
    ...
    okLocation () {
      ...
      //{{poiResult?poiResult.item.district: "未知区域"}} {{poiResult?poiResult.item.address: "未知地址"}} {{poiResult?poiResult.item.name: "未知地点"}} {{poiResult?poiResult.item.location.lat: "0.00"}},{{poiResult?poiResult.item.location.lng: "0.00"}}
      if (!this.poiResult) {
        return;
      }
